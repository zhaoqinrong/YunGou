...
SmartPhone Compatible web template, free WebDesigns for Nokia, Samsung, LG, SonyEricsson, Motorola web design"/>
    ...
    <!--//webfonts-->
    <!--animate-->
    <link href="${pageContext.request.contextPath}/jsp/admin/css/animate.css" rel="stylesheet" type="text/css"
          media="all">
            ...
            <div class="tables">
                <div class="bs-example widget-shadow" data-example-id="contextual-table">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                        <c:forEach items="${userpage.list}" var="user" varStatus="stus">
                            <c:choose>
                            ...
                            <c:if test="${user.status==0}">
                                <td name="status">未激活</td>
                            </c:if>
                            <c:if test="${user.status==1}">
                                <td name="status">激活</td>
                            </c:if>
                            <td>
                    ...
                    <nav class="col-md-offset-4">
                        <ul class="pagination pagination-lg">
                            ...
                            <c:if test="${userpage.currPage !=1}">
                                <li ><a href="adminUser?action=getPageUser&currPage=${userpage.currPage-1}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                            </c:if>
                            <c:forEach begin="1" end="${userpage.pages}" var="page">
                                <c:if test="${userpage.currPage==page}">
                                    <li class="active"><a href="adminUser?action=getPageUser&currPage=${page}">${page}</a></li>
                                </c:if>
                                <c:if test="${userpage.currPage !=page}">
                                    <li><a href="adminUser?action=getPageUser&currPage=${page}">${page}</a></li>
                                </c:if>
                            </c:forEach>
                            <c:if test="${userpage.currPage== userpage.pages}">
                                <li class="disabled"><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">»</span></a></li>
                            </c:if>
                            <c:if test="${userpage.currPage != userpage.pages}">
                                <li ><a href="adminUser?action=getPageUser&currPage=${userpage.currPage+1}" aria-label="Previous"><span aria-hidden="true">»</span></a></li>
                    ...
                    <div class="modal-body">
                        ...
                        <table class="col-md-offset-4">
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="mobile" placeholder="18401468727" class="input-xlarge">
                                </td>
                            </tr>
                        </table>
...
//       $("#sure1").attr("data-dismiss","modal");
    ...
    $(function () {
        ...
        $("#sure2").click(function () {
            ...
            $.get("${pageContext.request.contextPath}/adminUser?action=delById&id=" + userid, function (data) {
                if (data == "true") {
//                    history.go(0);
                    window.location.href = "adminUser?action=getPageUser&currPage=1"
                }
                $("#deluserid").val("");
            });
        })
