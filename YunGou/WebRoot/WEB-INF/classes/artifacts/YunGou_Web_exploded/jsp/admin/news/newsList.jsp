...
<div class="main-content">
    ...
    <jsp:include page="${pageContext.request.contextPath}/jsp/admin/nav.jsp"/>

    <!--left-fixed -navigation-->
    <!-- header-starts -->
    <!-- //header-ends -->
    <jsp:include page="${pageContext.request.contextPath}/jsp/admin/header.jsp"/>
    <div id="page-wrapper">
        <div class="main-page">
            <div class="tables">
                <div class="bs-example widget-shadow" data-example-id="contextual-table">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                    <c:if test="${not empty newsPages}">
                        <c:forEach items="${newsPages.list}" var="newsPage" varStatus="stus">
                            <c:choose>
                                <c:when test="${stus.index%2==0}">
                                    <tr class="active">
                                    <%-- &lt;%&ndash;有active success info warning danger&ndash;%&gt;--%>
                                </c:when>
                                <c:otherwise>
                                    <tr >
                                </c:otherwise>
                            ...
                            <th scope="row">${stus.index}</th>
                            <td name="className">${newsPage.className}</td>
                            <td name="id" hidden>${newsPage.id}</td>
                            <td name="title">${newsPage.title}</td>
                            <td name="content" hidden>${newsPage.content}</td>
                            <td name="createTime">${newsPage.createTime}</td>
                    ...
                    <nav class="col-md-offset-4">
                        <ul class="pagination pagination-lg">
                            <c:if test="${newsPages.currPage==1}">
                                <li class="disabled"><a href="javascript:void(0);" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                            </c:if>
                            <c:if test="${newsPages.currPage !=1}">
                                <li ><a href="${pageContext.request.contextPath}/news?action=getAllNewsAndClass&currPage=${newsPages.currPage-1}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                            </c:if>
                            <c:forEach begin="1" end="${newsPages.pages}" var="page">
                                <c:if test="${newsPages.currPage==page}">
                                    <li class="active"><a href="${pageContext.request.contextPath}/news?action=getAllNewsAndClass&currPage=${page}">${page}</a></li>
                                </c:if>
                                <c:if test="${newsPages.currPage !=page}">
                                    <li><a href="${pageContext.request.contextPath}/news?action=getAllNewsAndClass&currPage=${page}">${page}</a></li>
                                </c:if>
                            </c:forEach>
                            <c:if test="${newsPages.currPage== newsPages.pages}">
                                <li class="disabled"><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">»</span></a></li>
                            </c:if>
                            <c:if test="${newsPages.currPage != newsPages.pages}">
                                <li ><a href="${pageContext.request.contextPath}/news?action=getAllNewsAndClass&currPage=${newsPages.currPage+1}" aria-label="Previous"><span aria-hidden="true">»</span></a></li>
                    ...
                    <style>
                        ...
                        <table class="col-md-offset-4" >
                            ...
                            <tr>
                                ...
                                <td> <input type="text" name="mobile" placeholder="18401468727" class="input-xlarge"></td></tr>
                        </table>
